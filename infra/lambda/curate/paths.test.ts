...
describe('placementFromRawKey', () => {
  ...
  it('handles user-scoped raw keys (users/<id>/raw/projects/...)', () => {
    expect(placementFromRawKey('users/alice/raw/projects/X/notes.md')).toBe('projects/x');
  ...
  it('routes non-project top-level dirs to the dir name', () => {
    ...
    expect(placementFromRawKey('users/alice/raw/tools/cli.md')).toBe('tools');
  });

  it('falls back to inbox when there is no sub-folder', () => {
    expect(placementFromRawKey('raw/random-note.md')).toBe('inbox');
    expect(placementFromRawKey('users/alice/raw/note.md')).toBe('inbox');
  });
...
describe('sourcePageKey', () => {
  ...
  it('works for user-scoped prefixes', () => {
    expect(sourcePageKey('users/alice/generated/wiki/', 'people', 'john-abc12345'))
      .toBe('users/alice/generated/wiki/sources/people/john-abc12345.md');
  });
