...
| 029 | Implement the built-in OIDC auth gate (024 follow-up) | P2 | L | 024 | DONE (AUTH_MODE=none|oidc|proxy; proxy.ts page gate + requireSession() in-handler enforcement; login/callback/logout routes; auth-session encrypted cookie; deny-by-default allowlist; unit tests for auth, auth-session, auth-oidc, auth-url) |
