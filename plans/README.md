...
| 028 | HTML document fidelity & hardening (022 §10) | P3 | M | 022 | DONE — shared sanitize schema (`web/lib/sanitize-schema.ts`): inline style denylist filter, size-bounded `data:image/*` (no SVG), `FEATURE_IMAGE_PROXY` flag + `/api/image-proxy` route (SSRF-hardened, off by default), relative-link rewriting (query/fragment/`./`/`../` preservation), 3 styled HTML demo fixtures in `openwiki/html/`. 271 unit tests pass, build clean |
| 029 | Implement the built-in OIDC auth gate (024 follow-up) | P2 | L | 024 | DONE (AUTH_MODE=none|oidc|proxy; proxy.ts page gate + requireSession() in-handler enforcement; login/callback/logout routes; auth-session encrypted cookie; deny-by-default allowlist; unit tests for auth, auth-session, auth-oidc, auth-url. Allowlist later removed — IdP-delegated access, PR #81) |
| 030 | Mobile-responsive portal (drawer sidebar, adaptive overlays) | P2 | L | PR #82 merged | TODO |
