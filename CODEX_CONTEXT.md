...
- `aws-infrastructure.md` — Account `<account-id>`, bucket `<your-bucket>`, region `<aws-region>`
...
VAULT_PREFIX=<your-prefix>  (set in .env.local — but Lambda uses empty prefix)
VAULT_REGION=<aws-region>
BEDROCK_MODEL=amazon.nova-2-lite-v1:0  (or a cross-region profile, e.g. eu.amazon.nova-2-lite-v1:0)
```
