...
type EditorProps = {
  ...
  etag?: string;
  /**
   * Pre-seeded draft. Takes precedence over `doc` for initialization.
   * Used by the Ask-Wiki agent's post-hoc Save-as-page flow — the chat
   * panel hands the Editor a fully-formed draft (title + body) so the user
   * can review and tweak before the existing CRUD save path runs.
   */
...
export function Editor({ doc, docId, etag, initialDraft, onClose, onSave, showToast }: EditorProps) {
  const [title, setTitle] = useState(initialDraft?.title || doc?.title || 'Untitled');
  const [body, setBody] = useState(() => initialDraft?.body ?? buildInitialBody(doc));
  ...
  return (
    <>
      <div className="doc-toolbar">
        ...
        {initialDraft && (
          <span className="tag-chip" title="This draft was generated by the Ask-Wiki agent. Review before saving.">
            {ICONS.spark} drafted from chat
          </span>
        )}
