...
1. A `worker/` (or `api/workers/`) container runs the ingest pipeline, invocable via `canopy ingest <s3-key>`.
...
6. `canopy lint` validates generated output structure and frontmatter.
